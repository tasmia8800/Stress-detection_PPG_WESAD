...
def seperator_quad(df):
    ...
    try:
        meditation = grouped.get_group(4) # Check label? Quad usually 0,1,2,3... 
    except KeyError:
        meditation = pd.DataFrame()
...
def make_patient_data(subject_id, ma_usage):
    ...
    baseline, stress, amusement, meditation = seperator_quad(df)
    ...
    window_len_count = len(baseline_samples) + len(stress_samples) + len(amusement_samples) + len(meditation_samples)
...
def combine_files(subjects):
    ...
    try:
        ...
        if '0.0' in df.columns:
             df['label'] = (df['0.0'].astype(str) + df['1.0'].astype(str) + df['2.0'].astype(str) + df['3.0'].astype(str)).apply(lambda x: x.index('1'))
             df.drop(['0.0', '1.0', '2.0', '3.0'], axis=1, inplace=True)
